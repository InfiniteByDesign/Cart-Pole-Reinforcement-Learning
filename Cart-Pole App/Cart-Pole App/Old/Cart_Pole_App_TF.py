        ...
        for epoch in range(cfg.epochs):
            ...
            while fail==False and i < 60000:
                ...
                for update in range(cfg.run_state):
                    ...
                    state   = np.array(sm.cart_pole_model(dt,last_state,0,_u)).reshape(1,5)
                    X       = np.array([state[0,0],state[0,1],state[0,2],state[0,3]]).reshape(1,4)
                    
                    # Determine the success feedback, r
                    # state = [ang, ang vel, dist, vel]
                    angle = np.rad2deg(state[0,0])
                ...
                print("Epoch:", '%04d' % (epoch+1), "max was:", '%06d' % (max_i + 1), "steps, this epoch was:", '%06d' % (i + 1), "Ang:", "{:.3f}".format(angle), "force was:", '%03d' % (_u), "CTG:", "{:.5f}".format(critic_output[0,0]))
