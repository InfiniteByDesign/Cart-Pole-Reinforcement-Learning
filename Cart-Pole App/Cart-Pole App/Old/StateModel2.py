...
def cart_pole_model(dt,x,flag,u):
    ...
    # States:
    angle           = np.deg2rad(x[0,0])
    ang_vel         = np.deg2rad(x[0,1])
    dist            = x[0,2]
    x_vel           = x[0,3]
    ang_acc         = np.deg2rad(x[0,4])
    ...
    if useFriction == True:
        ...
        new_x_acc = (F + mp*l*(new_ang_vel**2*np.sin(angle) - new_ang_acc**2*np.cos(angle)) - uc*Nc*sgn) / (mc + mp)
        ...
        # New Angular Accelleration
        temp            = ( -F-mp*l*ang_vel**2*np.sin(angle))/(mc+mp) 
        new_ang_acc_num = g*np.sin(angle) + np.cos(angle)*temp    
        new_ang_acc_den = l*(4/3 - mp*np.cos(angle)**2/(mc+mp))    
        new_ang_acc     = new_ang_acc_num / new_ang_acc_den
        
        # Angular Updates
        new_ang_vel     = ang_vel + ang_acc * dt
        angle           = angle + ang_vel*dt + 0.5*ang_acc*dt**2
        angle           = angle# % (2*np.pi)
        
        # New Linear Accelleration
        new_x_acc = (F + mp*l*(ang_vel**2*np.sin(angle) - ang_acc**2*np.cos(angle))) / (mc + mp)
    ...
    return [np.rad2deg(angle),np.rad2deg(new_ang_vel),dist,new_x_vel,np.rad2deg(new_ang_acc)]
    """
    ...
    return [np.rad2deg(angle),np.rad2deg(new_ang_vel),dist,new_x_vel,np.rad2deg(new_ang_acc),temp,new_ang_acc_num,new_ang_acc_den,new_x_acc,sgn]